 ...
 */
var _a, _b;
import { getPricingTier } from '../pricingConfig.js';
// Mock registry for CLI functionality
const CONTAINER_REGISTRY = {
    'FileIntelligenceContainer': (_a = class FileIntelligenceContainer {
        ...
        _a),
    'JournalingContainer': (_b = class JournalingContainer {
            async execute(planName, input, sessionId) {
                return {
                    success: true,
...
class BMADOrchestrator {
     ...
     */
    async executeTrigger(trigger, planName, inputData) {
        try {
     ...
     */
    async executeContainer(containerName, planName, inputData) {
        try {
     ...
     */
    generateMockInput(containerName) {
        const mockData = {
    ...
    try {
        switch (command) {
            case 'pipeline':
                ...
                await orchestrator.runPipeline(args[1]);
                break;
            case 'container':
                if (args.length < 3) {
                ...
                await orchestrator.executeContainer(args[1], args[2]);
                break;
            case 'trigger':
                if (args.length < 3) {
                ...
                await orchestrator.executeTrigger(args[1], args[2]);
                break;
            case 'list':
                if (args.length < 3) {
                ...
                if (args[1] === 'containers') {
                    orchestrator.listContainers(args[2]);
                }
                else if (args[1] === 'features') {
                    orchestrator.listFeatures(args[2]);
                }
                else {
